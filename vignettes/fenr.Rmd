...
The input data for `fenr` consists of two tibbles or data frames. One contains functional term descriptions, the other term-feature mapping. These data can be obtain using helper functions `fetch_*` to download data from GO, KEGG, BioPlanet or WikiPathways. Alternatively, they can be provided by the user. The tibbles are converted into an object of class `fenr_terms`, which stores data in the format optimised for fast search and access. This object is then used by the function `functional_enrichment` to find enrichment for any given feature selection.
